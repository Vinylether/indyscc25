...
parser.add_argument("--numThreads", help="Number of threads", type=int, default=-1)
parser.add_argument("--skip", "-s", help="Number of events", type=int, default=0)
parser.add_argument("--rndSeed", help="Seed to use for random number generator", type=int, default=42)
...
rnd = acts.examples.RandomNumbers(seed=args.rndSeed)

s = acts.examples.Sequencer(
    events=args.events,
    skip=args.skip,
    trackFpes=False,
    numThreads=1 if args.geant4 else args.numThreads,
...
if args.edm4hep:
    ...
    if not args.ttbar:
        ...
        addPythia8(
            ...
            rnd=rnd,
            outputDirRoot=outputDir if (args.output_root and not args.onlyWriteVertices) else None,
            outputDirCsv=outputDir if (args.output_csv and not args.onlyWriteVertices) else None,
        )
...
logger = acts.logging.getLogger("Sequencer")
logger.info("Initial Random Seed: %d", args.rndSeed)
logger.info("Pileup: %d", args.ttbar_pu)
