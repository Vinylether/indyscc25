...
public:
	...
	uint64_t getMemoryLocation( const uint32_t x, const uint32_t y, const uint32_t z );
        bool isFinished();
        void completed();
		...
		{ "histoslots",       "Number of histogram slots to simulate result bins.", "32" },
		{ "meshx",	      "Mesh points in X", "32" },
		{ "meshy",   	      "Mesh points in Y", "32" },
